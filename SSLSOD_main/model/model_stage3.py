...
class RGBD_sal(nn.Module):
    ...
    def __init__(self):
        ...
        feats = list(models.vgg16_bn(weights=None).features.children())
        feats1 = list(models.vgg16_bn(weights=None).features.children())
    ...
    def forward(self, x,depth):
        ...
        certain_feature4 = F.interpolate(F.sigmoid(sideout5),size=e4_rgb.size()[2:], mode='bilinear')*e4_rgb * e4_depth
        ...
        uncertain_feature4 = self.fuse4_conv4(F.interpolate(F.sigmoid(sideout5),size=e4_rgb.size()[2:], mode='bilinear')*torch.abs(e4_rgb - e4_depth))
        fuse4 = self.fuse4_conv5(fuse4_certain + uncertain_feature4)
###
        fuse5_fpn = F.interpolate(fuse5, size=fuse4.size()[2:], mode='bilinear')
        fpn_certain_feature4 = F.interpolate(F.sigmoid(sideout5), size=fuse4.size()[2:], mode='bilinear') * fuse4 * fuse5_fpn
        ...
        fpn_uncertain_feature4 = self.fuse4_conv4_fpn(
            F.interpolate(F.sigmoid(sideout5), size=fuse4.size()[2:], mode='bilinear') * torch.abs(fuse4 - fuse5_fpn))
        ...
        certain_feature3 = F.interpolate(F.sigmoid(sideout4),size=e3_rgb.size()[2:], mode='bilinear')*e3_rgb * e3_depth
        ...
        uncertain_feature3 = self.fuse3_conv4( F.interpolate(F.sigmoid(sideout4),size=e3_rgb.size()[2:], mode='bilinear')*torch.abs(e3_rgb - e3_depth))
        fuse3 = self.fuse3_conv5(fuse3_certain + uncertain_feature3)
        ##
        output4_fpn = F.interpolate(output4, size=fuse3.size()[2:], mode='bilinear')
        fpn_certain_feature3 = F.interpolate(F.sigmoid(sideout4), size=fuse3.size()[2:], mode='bilinear') * output4_fpn * fuse3
        ...
        fpn_uncertain_feature3 = self.fuse3_conv4_fpn(
            F.interpolate(F.sigmoid(sideout4), size=fuse3.size()[2:], mode='bilinear') * torch.abs(fuse3 - output4_fpn))
        ...
        certain_feature2 = F.interpolate(F.sigmoid(sideout3),size=e2_rgb.size()[2:], mode='bilinear')*e2_rgb* e2_depth
        ...
        uncertain_feature2 = self.fuse2_conv4(F.interpolate(F.sigmoid(sideout3),size=e2_rgb.size()[2:], mode='bilinear')*torch.abs(e2_rgb - e2_depth))
        fuse2 = self.fuse2_conv5(fuse2_certain + uncertain_feature2)

        output3_fpn = F.interpolate(output3, size=fuse2.size()[2:], mode='bilinear')
        fpn_certain_feature2 = F.interpolate(F.sigmoid(sideout3), size=fuse2.size()[2:], mode='bilinear') * output3_fpn * fuse2
        ...
        fpn_uncertain_feature2 = self.fuse2_conv4_fpn(
            F.interpolate(F.sigmoid(sideout3), size=fuse2.size()[2:], mode='bilinear') * torch.abs(fuse2 - output3_fpn))
        ...
        certain_feature1 = F.interpolate(F.sigmoid(sideout2),size=e1_rgb.size()[2:],mode='bilinear')*e1_rgb * e1_depth
        ...
        uncertain_feature1 = self.fuse1_conv4(F.interpolate(F.sigmoid(sideout2),size=e1_rgb.size()[2:],mode='bilinear')*torch.abs(e1_rgb - e1_depth))
        fuse1 = self.fuse1_conv5(fuse1_certain+uncertain_feature1)

        output2_fpn = F.interpolate(output2, size=fuse1.size()[2:], mode='bilinear')
        fpn_certain_feature1 = F.interpolate(F.sigmoid(sideout2), size=fuse1.size()[2:],
        ...
        fpn_uncertain_feature1 = self.fuse1_conv4_fpn(
            F.interpolate(F.sigmoid(sideout2), size=fuse1.size()[2:], mode='bilinear') * torch.abs(fuse1 - output2_fpn))
