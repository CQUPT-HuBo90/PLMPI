...
class Crossmodal_Autoendoer(nn.Module):
    ...
    def forward(self, x,depth):
        ...
        output4_rgb = self.output4_rgb(F.interpolate(e5_rgb, size=e4_rgb.size()[2:], mode='bilinear')+e4_rgb)
        sideout4_rgbtod = self.sideout4_rgbtod(output4_rgb)
        output3_rgb = self.output3_rgb(F.interpolate(output4_rgb, size=e3_rgb.size()[2:], mode='bilinear') + e3_rgb)
        sideout3_rgbtod = self.sideout3_rgbtod(output3_rgb)
        output2_rgb = self.output2_rgb(F.interpolate(output3_rgb, size=e2_rgb.size()[2:], mode='bilinear') + e2_rgb)
        sideout2_rgbtod = self.sideout2_rgbtod(output2_rgb)
        sideout1_rgbtod = self.output1_rgbtod(F.interpolate(output2_rgb, size=e1_rgb.size()[2:], mode='bilinear') + e1_rgb)

        sideout5_dtorgb = self.sideout5_depthtorgb(e5_depth)
        output4_d = self.output4_depth(F.interpolate(e5_depth, size=e4_rgb.size()[2:], mode='bilinear')+e4_depth)
        sideout4_dtorgb = self.sideout4_depthtorgb(output4_d)
        output3_d = self.output3_depth(F.interpolate(output4_d, size=e3_rgb.size()[2:], mode='bilinear') + e3_depth)
        sideout3_dtorgb = self.sideout3_depthtorgb(output3_d)
        output2_d = self.output2_depth(F.interpolate(output3_d, size=e2_rgb.size()[2:], mode='bilinear') + e2_depth)
        sideout2_dtorgb = self.sideout2_depthtorgb(output2_d)
        sideout1_dtorgb = self.output1_depthtorgb(F.interpolate(output2_d, size=e1_rgb.size()[2:], mode='bilinear') + e1_depth)
