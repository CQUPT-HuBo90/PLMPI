...
pretrained_dict = torch.load(os.path.join('./pretext_task1.pth'))
...
pretrained_dict = torch.load(os.path.join('./pretext_task2.pth'))
model_dict = generator.state_dict()
pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
print(pretrained_dict.items())
model_dict.update(pretrained_dict)
...
## load data
image_root = '/home/user/chentaiyang/SSLSOD-main/RGBD_SOD_Datasets/Training_dataset/NJUD_NLPR_DUT_downstream_task_SOD/RGB/'
depth_root = '/home/user/chentaiyang/SSLSOD-main/RGBD_SOD_Datasets/Training_dataset/NJUD_NLPR_DUT_downstream_task_SOD/depth/'
gt_root = '/home/user/chentaiyang/SSLSOD-main/RGBD_SOD_Datasets/Training_dataset/NJUD_NLPR_DUT_downstream_task_SOD/GT/'
...
for epoch in range(1, opt.epoch+1):
    ...
    for i, pack in enumerate(train_loader, start=1):
        for rate in size_rates:
            ...
            b, c, h, w = gts.size()
            target_1 = F.interpolate(gts, size=h // 2, mode='nearest')
            target_2 = F.interpolate(gts,  size=h // 4, mode='nearest')
            target_3 = F.interpolate(gts,  size=h // 8, mode='nearest')
            target_4 = F.interpolate(gts,  size=h // 16, mode='nearest')
