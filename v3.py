...
class ResNet_Refine(nn.Module):
    def __init__(self, block, layer, is_local=True, num_classes=1000, inplanes=1024, planes=512, zero_init_residual=False,
                 groups=1, width_per_group=64, norm_layer=None):
        ...
        self.groups = groups
        self.base_width = width_per_group
        self.inplanes = inplanes  # 添加自定义输入通道数
        self.layer4 = self._make_layer(block, planes, layer, stride=2)  # 使用传入的planes
...
class ResNet_Attention(nn.Module):
    def __init__(self, block, layer, att_size=4,inplanes=1024, num_classes=1000, zero_init_residual=False,
...
class A_2_net(nn.Module):
    def __init__(self, code_length=12, num_classes=200, att_size=4, feat_size=2048, device='cpu', pretrained=True):
        ...
        self.attention1 = A_2_net_attention(att_size, inplanes=256)  # layer1 输出 256 通道
        self.attention2 = A_2_net_attention(att_size, inplanes=512)  # layer2 输出 512 通道
        self.attention3 = A_2_net_attention(att_size, inplanes=1024) # layer3 输出 1024 通道
