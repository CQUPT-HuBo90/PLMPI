...
def _make_levit_backbone(
    ...
    # 强制使用自动计算的hook位置（每个stage最后一个block）
    hooks = []
    idx = 0
    for stage in model.stages:
        hooks.append(idx + len(stage.blocks) - 1)
        idx += len(stage.blocks)

    # 注册hook
    all_blocks[hooks[0]].register_forward_hook(get_activation("1"))
    all_blocks[hooks[1]].register_forward_hook(get_activation("2"))
    all_blocks[hooks[2]].register_forward_hook(get_activation("3"))
...
def _make_pretrained_levit_384(pretrained, hooks=None):
    ...
    return _make_levit_backbone(model, hooks=None)